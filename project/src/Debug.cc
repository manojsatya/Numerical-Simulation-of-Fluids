...
namespace internal
{
                  ...
                  const char * const filename, int line )
   {
      if( !b )
      {
         std::cerr <<LIGHTRED<< "Check failed!\n\tFile:       " << filename << ":" << line << "\n"
   ...
   void infoFct( const std::string & message, const char * const filename, int line )
    {
       std::cerr <<CYAN<< "INFO!\n\tFile:       " << filename << ":" << line << "\n";
       std::cerr <<  "\t" << message << std::endl;
       std::cerr <<RESET_COLOR;
  ...
  void prg_level_fct(const std::string &message, const char * const filename, int line)  
   {
      std::cerr << LIGHTGREEN<<" Progress Level!\n\tFile:       " << filename << ":" << line << "\n";
      std::cerr <<  "\t" << message << std::endl;
      std::cerr << RESET_COLOR;
   }
