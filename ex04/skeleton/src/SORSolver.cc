...
int xbound{grid.p().getSize(0)-1};int ybound{grid.p().getSize(1)-1};
...
for(int i = 1 ; i < xbound ; ++i ) // Inner points only 
		for(int j = 1 ; j < ybound ; ++j) 
	// Residual = f{h} - A * p{h}
	res(i,j) = grid.rhs()(i,j) - invdx2 * (grid.p()(i+1,j) - 2.0 * grid.p()(i,j) + grid.p()(i-1,j)) 
				- invdy2 * (grid.p()(i,j+1) - 2.0 * grid.p()(i,j) + grid.p()(i,j-1));
	...
	std::cout << "Iteration:" << SORiter << "->	" << resNorm << std::endl;}  // Square root as mentioned in eq 4
