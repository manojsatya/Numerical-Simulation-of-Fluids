...
public:
	...
	inline bool checkParameter(const std::string & key) const;
...
{
	auto iterator = mapper.find(key);
    real value=0;
	CHECK_MSG(iterator != mapper.end(), "Sorry parameter '" + key +"' does not exist!");
...
inline bool FileReader::checkParameter(const std::string & key) const {
  if (mapper.find(key) != mapper.end()) return true ; else return false;  
}
