...
//std::cout << " Inside Compute FG_KE....first_f " << std::endl;
    ...
    //u2_y2(i,j) = idy2 * (grid_.u(i,j,NORTH) - 2 * grid_.u(i,j,CENTER) + grid_.u(i,j,SOUTH));
     ...
     third_g(i,j) = ((2/3) * idy) * (grid_.k(i,j,NORTH) - grid_.k(i,j,CENTER));
...
//std::cout << " Inside Compute FG_KE....first_f " << std::endl;
 ...
 void FluidSimulator::compute_RANSCoeff(){
    ...
    for( int j = 1 ; j <= jmax ; ++j){
        if(grid_.isFluid(i,j))// && grid_.isFluid(i+1,j))
        {
            //std::cout << "e"<<"("<<i<<","<<j <<")" << grid_.e(i,j,CENTER) << std::endl;
            if(grid_.e(i,j,CENTER)!=0)
            R_t = (grid_.k(i,j,CENTER) * grid_.k(i,j,CENTER))/(nu * grid_.e(i,j,CENTER));
            else
 ...
 void FluidSimulator::boundaryForKE(){
     ...
     for (int i = 1 ; i <=imax ; ++i){ // South
        //std::cout << "i = " << i << std::cout ;
         grid_.k()(i,0) = grid_.k_new()(i,0);
         grid_.e()(i,0) = grid_.e_new()(i,0);

     }

     for (int i = 1 ; i <=imax ; ++i){ // North
         grid_.k()(i,jmax + 1) = grid_.k_new()(i,jmax + 1);
         grid_.e()(i,jmax + 1) = grid_.e_new()(i,jmax + 1);

     }

     for (int j = 1 ; j <=jmax ; ++j){ //West

         grid_.k()(0,j)= grid_.k_new()(0,j);
         grid_.e()(0,j)= grid_.e_new()(0,j);

     }

     for (int j = 1 ; j <=jmax ; ++j){ // East

        grid_.k()(imax,j) = grid_.k_new()(imax,j);
        grid_.e()(imax,j) = grid_.e_new()(imax,j);
     }
...
void FluidSimulator::computeKE(){
    ...
    //Array<real> *k_new = &grid_.k_new();//pointer to new timestep buffers
   //Array<real> e_new = grid_.e_new();
...
//std::cout << "Hereeee" << std::endl;
    ...
    for (int i = 1 ; i <= imax ; ++i)
        ...
        //int i = 4; int j = 4;
                if(grid_.isFluid(i,j))// && grid_.isFluid(i,j+1))
                {
                    real f_nu_center = grid_.fnu()(i,j);
                    real f1_center = 1 + pow((0.05/f_nu_center),3);
                    ...
                    // Problem next line
                    real du_dy = (0.25 * idy) * (grid_.u(i,j,NORTH) + grid_.u(i-1,j,NORTH) - grid_.u(i,j,SOUTH) - grid_.u(i,j-1,WEST));
                ...
                //std::cout <<  grid_.k()(i,j) << std::endl;
                    ...
                    if(grid_.k()(i,j)<0){grid_.k()(i,j) = 0.0000001;}
                    ...
                    real five_e = (c_1 * f1_center * grid_.k()(i,j) * 0.5) * gradugradut;
               ...
               // else
                 //   grid_.k()(i,j) = 0.0; grid_.e()(i,j) = 0.0;
